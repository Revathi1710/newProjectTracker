...
import {
  selectCartItems, selectCartTotal, selectCartCount, selectCartLoading,
  removeFromCart, updateQuantity, clearCartAsync,
...
export default function CartPage() {
  ...
  // ✅ NO fetchCart here. Cart is fetched once by Header (in Layout).
  // Reading directly from Redux state is sufficient.
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 flex flex-col gap-4">
            {items.map((item) => {
              ...
              return (
                <div key={item.id} className="flex items-center gap-4 bg-white border border-slate-200 rounded-2xl p-4 shadow-sm hover:shadow-md hover:border-blue-200 transition-all">
                  <div className="w-20 h-20 rounded-xl overflow-hidden bg-blue-50 shrink-0">
                    {image
                      ? <img src={`${import.meta.env.VITE_SERVER_URL}/storage/${image}`} className="w-full h-full object-cover" alt={name} />
                ...
                {/*  <div className="flex items-center gap-2 shrink-0">
                    <button onClick={() => dispatch(updateQuantity({ id: item.id, quantity: item.quantity - 1 }))} className="w-8 h-8 bg-slate-100 hover:bg-blue-100 hover:text-blue-700 text-slate-600 rounded-lg font-bold transition-all">−</button>
                    <span className="w-6 text-center font-bold text-slate-800">{item.quantity}</span>
                    <button onClick={() => dispatch(updateQuantity({ id: item.id, quantity: item.quantity + 1 }))} className="w-8 h-8 bg-slate-100 hover:bg-blue-100 hover:text-blue-700 text-slate-600 rounded-lg font-bold transition-all">+</button>
                  </div>*/}
                  <button onClick={() => dispatch(removeFromCart(item.id))} className="text-slate-400 hover:text-red-500 p-2 transition-colors shrink-0">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white border border-slate-200 rounded-2xl p-6 sticky top-28 shadow-sm">
              ...
              <button onClick={() => navigate("/checkout")} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-extrabold py-4 rounded-xl transition-all shadow-md shadow-blue-200 active:scale-95">
                PROCEED TO CHECKOUT
              </button>
              <Link to="" className="block text-center text-sm text-slate-400 hover:text-blue-600 mt-4 transition-colors font-medium">
                ← Continue Shopping
              </Link>
            </div>
          </div>
