...
const TrashIcon = () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/></svg>;
const Spinner   = () => (
...
const ProjectRow = ({ index, item, onChange, onRemove }) => (
  <div className="border border-gray-200 rounded-xl p-4 bg-gray-50/60 flex flex-col gap-3">
    ...
    <div className="grid grid-cols-2 gap-3">
      <div>
        <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase tracking-wider">Project Promoter</label>
        <TextInput value={item.promoter} onChange={v => onChange("promoter", v)} placeholder="e.g. Shri. Sanjay Gupta" />
      </div>
      <div>
        <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase tracking-wider">Products & Capacity</label>
        <TextInput value={item.products_capacity} onChange={v => onChange("products_capacity", v)} placeholder="e.g. Grain based Ethanol (60 KLPD)" />
      </div>
      <div>
        <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase tracking-wider">Project Location</label>
        <TextInput value={item.location} onChange={v => onChange("location", v)} placeholder="e.g. Village- Biladi, Raipur, CG" />
      </div>
      <div>
        <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase tracking-wider">Proposed Investment</label>
        <TextInput value={item.investment} onChange={v => onChange("investment", v)} placeholder="e.g. 864 (Rs.Million)" />
      </div>
      <div>
        <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase tracking-wider">Project Completion</label>
        <TextInput value={item.completion} onChange={v => onChange("completion", v)} placeholder="e.g. June 2025" />
      </div>
    </div>
    <div>
      <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase tracking-wider">Executive Summary</label>
      <textarea value={item.summary} onChange={e => onChange("summary", e.target.value)}
        placeholder="Write the executive summary for this project..."
        rows={3}
...
export default function AddBlog() {
  ...
  const initialForm = {
    title: "", slug: "", category: "", blog_type: "", status: "published",
  ...
  const save = async () => {
    ...
    try {
      ...
      flash(`✓ "${data.data.title}" published successfully!`);
      setForm(initialForm);
  ...
  const CATEGORIES = [
    ...
    { value: "state", label: "State Reports" },
    { value: "industry", label: "Industry Reports" },
    { value: "region", label: "Region Reports" },
  ...
  const BLOG_TYPES = [
    { value: "top_projects", label: "Top Projects" },
    { value: "tracker", label: "Projects Tracker" },
        ...
        <div className="flex-1 overflow-y-auto px-7 py-7">
          <div className="max-w-[1260px] mx-auto">
            ...
            <div className="flex items-start justify-between mb-6">
              <div>
                <h1 className="text-xl font-bold text-gray-900">Add New Blog</h1>
                <p className="text-sm text-gray-400 mt-1">Create a new blog post for your site</p>
              </div>
              <div className="flex gap-2.5">
                <button type="button"
                  onClick={() => { setForm(initialForm); setProjects([emptyProject()]); removeImage(); setErrors({}); setSlugLocked(false); }}
            ...
            <div className="grid gap-5" style={{ gridTemplateColumns: "1fr 280px" }}>
              ...
              <div className="flex flex-col gap-5">
                ...
                <Card title="Basic Information">
                  <div className="mb-4">
                    <Field label="Blog Title *" error={errors.title}>
                      <TextInput value={form.title} onChange={handleTitleChange} placeholder="e.g. Top New Projects in Chhattisgarh - May 2023" error={errors.title} />
                    </Field>
                  </div>
                  ...
                  <div className="mb-4">
                    ...
                    {errors.slug
                      ...
                      : <p className="text-[11px] text-gray-400 mt-1">Auto-generated from blog title. You can edit it manually.</p>
                    }
                  </div>
                ...
                <Card title="Short Description">
                  <p className="text-xs text-gray-400 mb-3">Shown on the blog listing page below the title</p>
                  <RichArea value={form.short_description} onChange={set("short_description")}
                    placeholder="These are some of the top projects in this category covered in New Projects Tracker portal…"
                    minH={110} />
                ...
                <Card title="Project Details">
                  <p className="text-xs text-gray-400 mb-4">Add individual project entries — each becomes a block in the blog post</p>
                  <div className="flex flex-col gap-4">
                    {projects.map((proj, i) => (
                      <ProjectRow key={i} index={i} item={proj}
                        onChange={(k, v) => updateProject(i, k, v)}
                ...
                <Card title="Content Editor">
                  <p className="text-xs text-gray-400 mb-3">Additional content / closing sections (e.g. CTA, related links)</p>
                  <RichArea value={form.content} onChange={set("content")}
                    placeholder={"# Want to get more projects, buy,\n\n[Chhattisgarh Projects Tracker](https://...)\n\n### Recent Blog Posts"}
                    minH={160} />
                ...
                <Card title="SEO Settings">
                  <div className="flex flex-col gap-4">
                    <Field label="Meta Title" error={errors.meta_title}
                      hint="Leave blank to auto-use blog title">
                    ...
                    <Field label="Meta Description" error={errors.meta_description}
                      ...
                      <textarea value={form.meta_description} onChange={e => set("meta_description")(e.target.value)}
                        ...
                        className="w-full rounded-lg border border-gray-200 px-3 py-2.5 text-sm text-gray-700 bg-gray-50 outline-none focus:border-blue-500 focus:shadow-[0_0_0_3px_rgba(59,130,246,0.15)] transition-all resize-none"
                      />
                    ...
                    <Field label="Meta Keywords" error={errors.meta_keywords}
                      ...
                      <TextInput value={form.meta_keywords} onChange={set("meta_keywords")} placeholder="e.g. Chhattisgarh, projects, 2023, industrial" error={errors.meta_keywords} />
                    </Field>
                  </div>
                </Card>
                ...
                <Card title="Featured Image">
                  {!imagePreview ? (
                    ...
                    <div className="relative rounded-xl overflow-hidden border border-gray-200">
                      <img src={imagePreview} alt="Featured" className="w-full h-44 object-cover block" />
                      ...
                      <button type="button" onClick={() => fileRef.current.click()}
                        className="mt-0 w-full text-xs py-2 bg-white text-gray-500 font-medium hover:bg-gray-50 transition-colors border-t border-gray-200">
                        ↺ Replace Image
                      </button>
                    </div>
                  )}
